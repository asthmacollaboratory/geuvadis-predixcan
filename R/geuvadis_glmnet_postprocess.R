...
## colnames(data) = c("Gene", "SubjectID", "Predicted_Expr", "Measured_Expr")
#compute.gene.lm = function(data, out.file){
#    sink(out.file)
#    cat("Gene\tP.value\tR2\tSpearman.rho\n")
#    for(gene in unique(sort(data$Gene))){
...
#        my.rho = cor.test(data$Predicted_Expr, data$Measured_Expr, method = "spearman")$estimate
