...
path.to.samples = "/netapp/home/kkeys/gala_sage/rnaseq/geuvadis/igsr_samples.tsv"
...
for (pop in pops) {
	...
	x.num = dim(x.sub)[1]
	filename = paste0("geuvadis.", tolower(pop), x.num, ".sampleids.txt")
...
for (pop in pops) {
	x.sub = x[!c(pop),c("Sample name", "Sample name")]
	x.num = dim(x.sub)[1]
	filename = paste0("geuvadis.not", tolower(pop), ".sampleids.txt")
	fwrite(x.sub, file = filename, col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
}

# parse the complement of YRI + FIN
x.sub = x[!c("YRI", "FIN"),c("Sample name", "Sample name")]
filename = paste0("geuvadis.eur278.sampleids.txt")
fwrite(x.sub, file = filename, col.names = FALSE, row.names = FALSE, quote = FALSE, sep = "\t")
