...
compute.new.weights = function(){
    ...
    y.df.path = args[2] # points to data frame file of expression levels
    ...
    nfolds    = args[9] # number of folds to use for crossvalidation, e.g. 10
    ...
    # N.B. this script assumes that y.df is a data frame with column format "GENE,SAMPLE1,SAMPLE2,...,SAMPLEN" containing inverse normal rank-norm'd RPKMs 
        ...
        if(is.null(dim(X)) | dim(X)[2] == 0){
            ...
            if (length(y.idx) != 0){
                ...
                if (length(y) > 0){
                    ...
                    # in glmnet, we do k-fold CV, where k = nfolds 
                    #k = dim(X)[1]
                    #k = 10
                    ...
                    #my.glmnet = cv.glmnet(x=X, y=y, nfolds=nfolds, alpha=alpha, keep=TRUE, grouped=FALSE, dfmax=n, pmax=n, lambda=lambda, penalty.factor=penalty) # cannot group if <3 obs per fold, which is the case with LOOCV
                    #my.glmnet = cv.glmnet(x=X, y=y, nfolds=nfolds, family = "gaussian", alpha=alpha, keep=TRUE, grouped=FALSE, dfmax=n, pmax=n, lambda=lambda)#, penalty.factor=penalty)
                    my.glmnet = cv.glmnet(x=X, y=y, nfolds=nfolds, family = "gaussian", alpha=alpha, keep=TRUE, grouped = FALSE, dfmax=n, pmax=n, lambda.min.ratio = 0.01, nlambda=nlambda)#, penalty.factor=penalty)
